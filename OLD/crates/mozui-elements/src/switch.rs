...
impl Element for Switch {
    ...
    fn paint(&mut self, bounds: Rect, cx: &mut PaintContext) {
        ...
        // Track with inset shadow for recessed look
        let inset = Shadow::inset(0.0, 1.0, 2.0, 0.0, Color::rgba(0, 0, 0, 0.25));
        cx.draw_list.push(DrawCommand::Rect {
            bounds: track_bounds,
            background: Fill::Solid(track_color.with_alpha(alpha)),
            corner_radii: Corners::uniform(track_radius),
            border: None,
            shadow: None, shadows: vec![inset],
        });

        // Thumb with drop shadow for elevation
