...
impl Element for Button {
    ...
    fn paint(&mut self, bounds: Rect, cx: &mut PaintContext) {
        ...
        // Shadow: show on non-disabled, non-active states for raised variants
        let is_active = cx.interactions.is_active(bounds);
        let shadow = if self.colors.shadow && !self.disabled && !is_active {
            Some(self.shadow_sm)
        } else {
            None
        };
