...
impl DatePickerState {
    ...
    fn clean(&mut self, _: &mozui::ClickEvent, window: &mut Window, cx: &mut Context<Self>) {
        cx.stop_propagation();
        match self.date {
            Date::Single(_) => {
    ...
    fn toggle_calendar(&mut self, _: &mozui::ClickEvent, _: &mut Window, cx: &mut Context<Self>) {
        self.open = !self.open;
        cx.notify();
    }
...
impl Render for DatePickerState {
    fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl mozui::IntoElement {
        Empty
    }
}
