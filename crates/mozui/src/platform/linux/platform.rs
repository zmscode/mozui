...
pub(crate) const KEYRING_LABEL: &str = "mozui-github-account";
...
impl<P: LinuxClient + 'static> Platform for LinuxPlatform<P> {
    ...
    fn restart(&self, binary_path: Option<PathBuf>) {
        ...
        let restart_process = std::process::Command::new("/usr/bin/env")
            .arg("bash")
            .arg("-c")
            .arg(script)
            .arg(&app_pid)
        ...
        self.background_executor().spawn(async move {
            ...
            for item in items.into_iter() {
                if item.label().await.is_ok_and(|label| label == KEYRING_LABEL) {
                    ...
                    // a current limitation of mozui's credentials api
...
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(super) fn modifiers_from_xkb(keymap_state: &State) -> crate::Modifiers {
...
#[cfg(any(feature = "wayland", feature = "x11"))]
pub(super) fn capslock_from_xkb(keymap_state: &State) -> crate::Capslock {
