...
pub fn test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let Ok(func) = parse2::<ItemFn>(item) else {
        return quote_spanned! { item_span =>
            compile_error!("#[mozui::property_test] must be placed on a function");
        };
    };
    ...
    let proptest_args = quote! {
        #[strategy = ::mozui::seed_strategy()] __seed: u64,
        #proptest_args
    };
    ...
    let run_test_body = match &asyncness {
        ...
        Some(_) => quote! {
            let foreground_executor = mozui::ForegroundExecutor::new(std::sync::Arc::new(dispatcher.clone()));
            #cx_vars
    ...
    quote! {
        ...
        #[::mozui::proptest::property_test(proptest_path = "::mozui::proptest", #args)]
        #(#outer_fn_attributes)*
...
fn remove_cxs(parsed: &mut ParsedArgs, args: &mut Vec<FnArg>, test_name: &Ident) {
    ...
    args.retain_mut(|arg| {
        ...
        parsed.cx_vars.extend(quote!(
            let mut #cx_varname = mozui::TestAppContext::build(
                dispatcher.clone(),
...
fn remove_background_executor(parsed: &mut ParsedArgs, args: &mut Vec<FnArg>) {
    args.retain_mut(|arg| {
        ...
        parsed
            .inner_fn_args
            .extend(quote!(mozui::BackgroundExecutor::new(std::sync::Arc::new(
                dispatcher.clone()
            )),));
