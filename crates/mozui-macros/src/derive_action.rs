...
pub(crate) fn derive_action(input: TokenStream) -> TokenStream {
    ...
    */
    ...
    let build_fn_body = if no_json {
        ...
        quote! { Err(mozui::private::anyhow::anyhow!(#error_msg)) }
    } else if is_unit_struct {
        quote! { Ok(Box::new(Self)) }
    } else {
        quote! { Ok(Box::new(mozui::private::serde_json::from_value::<Self>(_value)?)) }
    };

    let json_schema_fn_body = if no_json || is_unit_struct {
        quote! { None }
    } else {
        quote! { Some(<Self as mozui::private::schemars::JsonSchema>::json_schema(_generator)) }
    };
    ...
    TokenStream::from(quote! {
        ...
        impl mozui::Action for #struct_name {
            ...
            fn partial_eq(&self, action: &dyn mozui::Action) -> bool {
                action
                    .as_any()
                    .downcast_ref::<Self>()
            ...
            fn boxed_clone(&self) -> Box<dyn mozui::Action> {
                Box::new(self.clone())
            }

            fn build(_value: mozui::private::serde_json::Value) -> mozui::Result<Box<dyn mozui::Action>> {
                #build_fn_body
            }

            fn action_json_schema(
                _generator: &mut mozui::private::schemars::SchemaGenerator,
            ) -> Option<mozui::private::schemars::Schema> {
                #json_schema_fn_body
            }
