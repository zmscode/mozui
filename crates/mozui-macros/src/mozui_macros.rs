...
///
/// ```compile_fail
/// # #[macro_use] extern crate mozui_macros;
/// # #[macro_use] extern crate mozui;
/// #[derive(AppContext)]
/// struct MyContext<'a> {
///     app: &'a mut mozui::App
/// }
...
/// # Arguments
///
/// - `#[mozui::test]` with no arguments runs once with the seed `0` or `SEED` env var if set.
/// - `#[mozui::test(seed = 10)]` runs once with the seed `10`.
/// - `#[mozui::test(seeds(10, 20, 30))]` runs three times with seeds `10`, `20`, and `30`.
/// - `#[mozui::test(iterations = 5)]` runs five times, providing as seed the values in the range `0..5`.
/// - `#[mozui::test(retries = 3)]` runs up to four times if it fails to try and make it pass.
/// - `#[mozui::test(on_failure = "crate::test::report_failure")]` will call the specified function after the
///   tests fail so that you can write out more detail about the failure.
///
/// You can combine `iterations = ...` with `seeds(...)`:
/// - `#[mozui::test(iterations = 5, seed = 10)]` is equivalent to `#[mozui::test(seeds(0, 1, 2, 3, 4, 10))]`.
/// - `#[mozui::test(iterations = 5, seeds(10, 20, 30)]` is equivalent to `#[mozui::test(seeds(0, 1, 2, 3, 4, 10, 20, 30))]`.
/// - `#[mozui::test(seeds(10, 20, 30), iterations = 5]` is equivalent to `#[mozui::test(seeds(0, 1, 2, 3, 4, 10, 20, 30))]`.
///
...
/// `Arbitrary`. Parameters to a `#[mozui::property_test]`, by default, use a
...
/// ## Scheduler
///
/// Similar to `#[mozui::test]`, this macro will choose random seeds for the test
...
///
/// ```ignore
/// pub fn methods::<T: TheTrait + 'static>() -> Vec<mozui::inspector_reflection::FunctionReflection<T>>;
///
/// pub fn find_method::<T: TheTrait + 'static>() -> Option<mozui::inspector_reflection::FunctionReflection<T>>;
/// ```
///
